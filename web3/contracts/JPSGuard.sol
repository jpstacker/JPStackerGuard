...
// File: contracts/interfaces/IERC4671.sol
...
// File: contracts/interfaces/IERC4671Metadata.sol
...
// File: contracts/interfaces/IERC4671Enumerable.sol
...
// File: contracts/base/ERC4671.sol
    ...
    ERC165
{
    ...
    uint256 private _emittedCount = 1;
...
contract JPSGuard is ERC4671, Ownable {
    ...
    event WhitelistUpdated(address indexed _user, uint256 indexed _serviceId, bool indexed _status);
    ...
    error JPSG_EmptyURI();
    error JPSG_InactiveService();
    error JPSG_InvalidServiceID();
    error JPSG_NotPaidService();
    ...
    */
    modifier checkService(
        ...
        require(_serviceId <= serviceCount, JPSG_InvalidServiceID());           // Is Valid Serice ID
        require(services[_serviceId].active, JPSG_InactiveService());                // Is Service Active
        require(services[_serviceId].status == _status, JPSG_InvalidTokenId()); // Is status correct
        _;
    }
    ...
    */
        ...
        checkService(_serviceId, ServiceStatus.Paid) 
    {
        ...
        _mintToken(_user, _serviceId);
        payable(services[_serviceId].manager).transfer(managerAmount);
    }
    ...
    function revokeToken(
        ...
        external 
        onlyManager(serviceIds[_tokenId]) // It checks TokenId, ServiceId, Is Manager
    { 
        require(services[serviceIds[_tokenId]].status != ServiceStatus.Paid, JPSG_NotPaidService());
        require(isValid(_tokenId), JPSG_InvalidTokenId());
        require(ownerOf(_tokenId) == _user, JPSG_NotTokenOwner());
        _revoke(_tokenId);
    }

    function revokeOwnToken(
        uint256 _tokenId
    ) 
        external 
    { 
        require(isValid(_tokenId), JPSG_InvalidTokenId());
        require(ownerOf(_tokenId) == msg.sender, JPSG_NotTokenOwner());
        _revoke(_tokenId);
    }
        ...
        returns (bool) 
    {
        ...
        emit WhitelistUpdated(_user, _serviceId, _status);
        return true;
    }
        ...
        returns (bool) 
    {
        for (uint256 i = 0; i < _users.length; i++) {
            ...
            emit WhitelistUpdated(_users[i], _serviceId, _status);
        }
        return true;
    }
